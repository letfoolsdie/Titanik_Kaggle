# -*- coding: utf-8 -*-
"""
Created on Tue Jan 13 13:37:53 2015
...
traindf = df.drop(['Name', 'Age', 'SibSp', 'Parch', 'Ticket', 'Fare', 'Cabin', 'Embarked'], axis = 1)
...
#testdf = testdf.drop(['Name', 'Age', 'SibSp', 'Parch', 'Ticket', 'Fare', 'Cabin', 'Embarked'], axis = 1)
...
outvectors = [[1.0, test_data[i][0], test_data[i][1]] for i in range(len(test_data))]
...
for i in range(len(hypov)):
    if hypov[i] != test_value[i]:
        err += 1
print(err/len(hypov))

#
#prediction_file = open('LR_w_class_and_sex_py.csv', 'w', newline = '')
