...
survival_table = np.zeros([2,num_of_classes, num_of_brackets],float)
...
train_array[train_array[0:,5] > 2, 5] = 3

for i in range(num_of_classes):
    for m in range(num_of_brackets):
        women_stats = train_array[(train_array[0:,3] == 0) & (train_array[0:, 2] == i+1) \
         & (train_array[0:,4] >= (m*fare_bracket_size)) & (train_array[0:,4] < ((m+1)*fare_bracket_size)),1]
        men_stats = train_array[(train_array[0:,3] == 1) & (train_array[0:, 2] == i+1) \
        & (train_array[0:,4] >= (m*fare_bracket_size)) & (train_array[0:,4] < ((m+1)*fare_bracket_size)),1]
        
        survival_table[0,i,m] = sum(women_stats) / len(women_stats) if len(women_stats)>0 else 0
        survival_table[1,i,m] = sum(men_stats) / len(men_stats) if len(men_stats)>0 else 0
...
#    hypov = np.append(hypov, survival_table[i[3]] [i[2]-1] [int(i[4]/10)])
#
#
#
...
#    np.append(hypov, survival_table[i[3]] [i[2]] [i[5]] [int(i[4]/10)])
#    np.append(hypov, survival_table[int(i[3])] [int(i[2])] [int(i[5])] [int(i[4]/10)])
    hypov = np.append(hypov, survival_table[i[2]] [i[1]-1] [int(i[3]/10)])


prediction_file = open('group_aver_gend_rltv_fare_class111.csv', 'w', newline = '')
